...
void processLine(chessEngine& engineState, std::string_view inputLine) {
    if (inputLine == "uci") {
        ...
        bool valid = engineState.stopSearching();
        // we handle the case when the engine isnt runnning so we avoid crashing the gui if the user presses the wrong button 
