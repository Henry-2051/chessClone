...
//     {AlgebraicChessInput::Capture,              [](char _){return CaptureStatus::Capture; },},
...
std::array<std::optional<std::variant<outputFunction, EndChar>>, 11> outputArray{{
    ...
    [](char _){ return CaptureStatus::Capture; },     // Index 3: Capture
...
std::optional<std::pair<chessMove, S>> processSingleMove(std::string move) {
    S state = S::S;
    stateTransitionFunction transFunction = chessTransitionFunction;
    outputFunctionFunctionType thisOutputFunctionFunction = outputFunctionFunction;

    chessMove theMove{};

    auto handleCastlingByState = [](chessMove move, S state){
        if (state == S::CS3) {
            auto maybeMove = move.pushNewInformation(CastlingStatus::Short);
            if (!maybeMove) {
                throw std::invalid_argument("error cant castle twice in 1 move");
            }
            return maybeMove.value();
        } else if (state == S::CS5) {
            auto maybeMove = move.pushNewInformation(CastlingStatus::Long);
            if (!maybeMove) {
                throw std::invalid_argument("error cant castle twice in 1 move");
            }
            return maybeMove.value();
    ...
    for (char character : move) {
        AlgebraicChessInput typeOfCharacter = charToTokenType.at(character);

        state = transFunction({state, typeOfCharacter});

        if (state == S::_) {
            return std::nullopt;
        }

        std::optional<std::variant<outputFunction, EndChar>> optionalOutputFunctionOrEndCharacter = thisOutputFunctionFunction(typeOfCharacter);


        // if this is false then we're castling
        if (optionalOutputFunctionOrEndCharacter) {
            // if we're at the end of the move 
            if (std::holds_alternative<EndChar>(optionalOutputFunctionOrEndCharacter.value())) {
                goto EndOfFunction;
            }
            else {
                ...
                if (state == S::PP2) {
                    outFunc = [](char c) {
                        switch (c) {
                        case 'Q':
                            return pawnPromotion(ChessPieces::Queen);
                        case 'R':
                            return pawnPromotion(ChessPieces::Rook);
                        case 'B':
                            return pawnPromotion(ChessPieces::Bishop);
                        case 'N':
                            return pawnPromotion(ChessPieces::Knight);
                        }
                        return pawnPromotion(ChessPieces::Pawn);
                    };
                } else {
                    outFunc = std::get<outputFunction>(optionalOutputFunctionOrEndCharacter.value());
                }

                outputChange changeInMove = outFunc(character);
                // the use of auto here creates a seperate function for each of the variants in outputChange, pushNewInformation is overloaded for each of these but not the variant
                // itself so std::visit just unpacks the variant and calls pushNewInformation on the underlying type, without using a large if else block
                theMove = std::visit(
                    [theMove](auto&& information)->std::optional<chessMove>{
                        return theMove.pushNewInformation(information);
                    }
                    ,changeInMove)
                .value_or(theMove);
            }
...
EndOfFunction:
    theMove = handleCastlingByState(theMove, state);
    theMove = theMove.promoteMoveToMaximum().value_or(theMove);
    return {{theMove, state}};
}
...
std::pair<std::string, std::string> split_by_delimiter(std::string input, char delim) {
    ...
    input = input.substr(first_delim_idx);

    return {newstring, input};
}

auto splitnewline = [](std::string& s)->std::pair<std::string, std::string>{
    auto [first, second] = split_by_delimiter(s, '\n');
    trim(first);
    trim(second);
    return {first, second};
};

std::pair<std::string, std::string> split_by_newline_or_space(std::string input) {
    trim(input);
    {
        auto [first, second] = split_by_delimiter(input, ' ');
        if (first != "") {
            return {first, second};
        }
    }
...
struct printCtx{
    ...
    bool _alreadyWriting = false;

    printCtx(PrintMode mode, std::string file_path=""): mode(mode), file_path(file_path) {}

    void printMove(chessMove move, std::string_view move_text) {
        switch (mode){
            case PrintMode::Print:
                ...
                std::cout << std::endl;
            break;
            case PrintMode::WriteToFile:
            {
                std::string to_write = std::format("{} : {}\n", move_text, move.moveToString());
                std::ofstream file;
                if (_alreadyWriting) {
                ...
                file << to_write;
                file.close();
                _alreadyWriting = true;
            }
            break;
            case PrintMode::Nothing:
            break;
        }
    }
};
...
std::vector<chessMove> decodeChessGameBetter(std::string inputString, printCtx ctx) {
    std::vector<chessMove> moves {};
    std::string moveString;
    auto pair_result = split_by_newline_or_space(inputString);
    while (pair_result.first != "") {

        std::optional<std::pair<chessMove, S>> result = processSingleMove(pair_result.first);

        moves.push_back(result.value().first);
        ctx.printMove(result.value().first, pair_result.first);

        pair_result = split_by_newline_or_space(pair_result.second);
    }
    return moves;
}
