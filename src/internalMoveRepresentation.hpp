...
namespace ChessMoveHelpers {
    auto stringPiece = [](ChessPieces piece) {
        switch (piece) {
    ...
    auto squareToString = [](std::optional<chessSquare> cSquare) {

        std::string squareToString = "";
        if (cSquare) {
            if (cSquare->file) {
                squareToString.push_back('a' + static_cast<char>(*cSquare->file));
            }
            else {
                squareToString += "<noFile>";
            }

            if (cSquare->rank) {
                squareToString.push_back('1' + static_cast<char>(*cSquare->rank));
            }
            else {
                squareToString += "<noRank>";
            }
        }
        else {
            return std::string("<NoSquare>");
        }
        return squareToString;
    };

    auto stringCheckStatus = [](CheckStatus checkStatus) {
        switch (checkStatus) {
            case CheckStatus::Check:
                return "check";
            case CheckStatus::Checkmate:
                return "checkmate";
            case CheckStatus::False:
                return "nocheck";
        }
    };

    auto stringCastlingStatus = [](CastlingStatus castlingStatus) {
        switch (castlingStatus) {
            case CastlingStatus::False:
                return "NoCastling";
            case CastlingStatus::Short:
                return "Short";
            case CastlingStatus::Long:
                return "Long";
        }
    };

    auto stringPromotionStatus = [](PromotionStatus promotionStatus) {
        switch (promotionStatus) {
            case PromotionStatus::False:
                return "NoPromotion";
            case PromotionStatus::Promotion:
                return "Promotion";
        }
    };

    auto stringCaptureStatus = [](CaptureStatus captureStatus) {
        switch (captureStatus) {
            case CaptureStatus::Capture:
                return "Capture";
            case CaptureStatus::False:
                return "NoCapture";
        }
    };
}
...
struct chessMove
{
    ...
    void printMove() const {
        ...
        std::cout << std::format("Piece : {}\n", ChessMoveHelpers::stringPiece(piece));

        std::cout << std::format("moveFrom : {}\n", ChessMoveHelpers::squareToString(moveFrom));

        std::cout << std::format("moveTo : {}\n", ChessMoveHelpers::squareToString(moveTo));

        std::cout << std::format("checkStatus : {}\n", ChessMoveHelpers::stringCheckStatus(checkStatus));

        std::cout << std::string("pawnPromotion : ") + ChessMoveHelpers::stringPiece(pawnPromotion) + std::string(pawnPromotion == ChessPieces::King ? " ERROR PROMOTION TO KING" : "") + "\n";

        std::cout << std::format("castlingStatus : {}\n", ChessMoveHelpers::stringCastlingStatus(castlingStatus));

        std::cout << std::format("captureStatus : {}\n", ChessMoveHelpers::stringCaptureStatus(captureStatus));

        std::cout << std::format("promotionStatus : {}\n", ChessMoveHelpers::stringPromotionStatus(promotionStatus));
    }

    std::string moveToString() const {
        std::string result = "{";

        result += std::format("Piece : {}, ",
            ChessMoveHelpers::stringPiece(piece));

        result += std::format("moveFrom : {}, ",
            ChessMoveHelpers::squareToString(moveFrom));

        result += std::format("moveTo : {}, ",
            ChessMoveHelpers::squareToString(moveTo));

        result += std::format("checkStatus : {}, ",
            ChessMoveHelpers::stringCheckStatus(checkStatus));

        result += std::string("pawnPromotion : ") +
            ChessMoveHelpers::stringPiece(pawnPromotion) +
            (pawnPromotion == ChessPieces::King ? " ERROR PROMOTION TO KING" : "") +
            ", ";

        result += std::format("castlingStatus : {}, ",
            ChessMoveHelpers::stringCastlingStatus(castlingStatus));

        result += std::format("captureStatus : {}, ",
            ChessMoveHelpers::stringCaptureStatus(captureStatus));

        result += std::format("promotionStatus : {}",
            ChessMoveHelpers::stringPromotionStatus(promotionStatus));
    ...
    chessMove(const chessMove& move) : piece(move.piece), moveFrom(move.moveFrom), moveTo(move.moveTo), checkStatus(move.checkStatus), pawnPromotion(move.pawnPromotion), castlingStatus(move.castlingStatus), captureStatus(move.captureStatus), promotionStatus(move.promotionStatus), m_lastPushed(move.m_lastPushed), m_currentSquareAdd(move.m_currentSquareAdd), hasMovementCollapsed(move.hasMovementCollapsed){};
    ...
    std::optional<chessMove> pushNewInformation(CastlingStatus status) const {
        if (castlingStatus != CastlingStatus::False) {
            return std::nullopt;
        }
        chessMove newMove = chessMove(*this);
    ...
    std::optional<chessMove> pushNewInformation(struct pawnPromotion piece) const {
        ...
        if (piece.piece!=ChessPieces::Pawn) {
            newMove.promotionStatus = PromotionStatus::Promotion;
        }
